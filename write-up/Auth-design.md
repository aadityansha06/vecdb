...
For generating the key, I was looking at the Linux ```/dev/urandom``` interface since it provides cryptographically secure random bytes. A 256-bit value can be generated from 32 random bytes, and for the initial implementation even something like``` head -c 32 /dev/urandom | sha256sum``` is enough to demonstrate the idea. Eventually, this can be handled directly inside the C implementation rather than relying on a shell command.
