...
search_req_t *parse_search_request(const char *json_body) {
  ...
  search_req_t *req = malloc(sizeof(search_req_t));
...
insert_req_t *parse_insert_request(const char *json_body) {
    ...
    cJSON *metadata = cJSON_GetObjectItemCaseSensitive(json, "metadata"); // Optional

    if (!cJSON_IsString(db_name) || !cJSON_IsNumber(id) || !cJSON_IsArray(vec_array)) {
        cJSON_Delete(json);
        return NULL;
    }

    insert_req_t *req = malloc(sizeof(insert_req_t));
    strncpy(req->db_name, db_name->valuestring, sizeof(req->db_name) - 1);
    ...
    for (uint64_t i = 0; i < req->dimension; i++) {
        ...
        if (!cJSON_IsNumber(item)) {
            free(req->vector);
            if (req->metadata) free(req->metadata);
            free(req);
            cJSON_Delete(json);
            return NULL;
        }
...
train_req_t *parse_train_request(const char *json_body) {
    ...
    cJSON *max_iterations = cJSON_GetObjectItemCaseSensitive(json, "max_iterations");

    if (!cJSON_IsString(db_name) || !cJSON_IsNumber(k) || !cJSON_IsNumber(max_iterations)) {
        cJSON_Delete(json);
        return NULL;
    }

    train_req_t *req = malloc(sizeof(train_req_t));
    strncpy(req->db_name, db_name->valuestring, sizeof(req->db_name) - 1);
